...
save()
{
  ...
  this.editForm['date_vente'] = this.date_vente_inv;
   ...
   /*--------------------------*/
   let num_vente = this.editForm.controls['num_vente'].value;
   /*--------------------------*/
  ...
  else
  {
    ...
    this.logForm.patchValue({
      ...
      action: 'Vente N° '+num_vente+' effectué par '+this.agent_name+' | '+this.current_date
    });
...
resetForm() {
  ...
  let year =  2022;
  this.httpclient.get<any>(this.base_url+'/countVentes/'+year).subscribe(
    response => {
      //console.log(response);
      let nbr = response[0]['nbr'];
      this.num_vente = sprintf('%05d', nbr);
    }
  );
