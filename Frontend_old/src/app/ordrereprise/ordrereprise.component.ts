...
import { Ordrereprise } from '../models/ordrereprise.model';
import { LoginService } from '../services/login.service';

@Component({
  selector: 'app-ordrereprise',
  templateUrl: './ordrereprise.component.html',
  styleUrls: ['./ordrereprise.component.css']
})
export class OrdrerepriseComponent implements OnInit {
  base_url : string;
  ...
  getAllOrdre(){
       this.httpclient.get<any>(this.base_url+'/getAllOrdrerep').subscribe(
      response => {
        console.log(response);
  ...
  onSubmit(f: NgForm) {
    const url = this.base_url+'/createOrdrerep';
  ...
  openDetails(targetModal, ordre: Ordrereprise) {
    ...
    document.getElementById('date_suspension_detail').setAttribute('value', ordre.date_reprise);
  ...
  openEdit(targetModal, ordre: Ordrereprise) {
    this.modalService.open(targetModal, {
      backdrop: 'static',
      size: 'lg'
    });
    this.editForm.patchValue( {
      ...
      ref: ordre.ref,
      date_reprise: ordre.date_reprise,
  ...
  onSave() {
    const editURL = this.base_url+'/updateOrdrerep/' + this.editForm.value.id;
  ...
  openDelete(targetModal, ordre: Ordrereprise) {
  ...
  onDelete() {
    const deleteURL = this.base_url+'/removeOrdrerep/' +this.deleteId;
